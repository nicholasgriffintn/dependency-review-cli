...
describe("OutputFormatter", () => {
	...
	it("should include scorecard data in JSON output", () => {
		...
		assert.strictEqual(
			parsed.scorecard.dependencies[0].change.name,
			"test-pkg",
		);
	...
	it("should include snapshot warnings in output", () => {
		...
		assert(
			output.includes("Warning: Some packages may not have complete data"),
		);
	});
	...
	it("should group vulnerabilities by package and deduplicate", () => {
		...
		assert(
			vulnMatches && vulnMatches.length <= 2,
			"Should deduplicate identical vulnerabilities",
		);
