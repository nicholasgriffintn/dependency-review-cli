...
describe("Scorecard", () => {
	it("should get scorecard data for GitHub repositories", async () => {
		...
		mockFetch(
			"https://api.securityscorecards.dev/projects/github.com/test/test-package",
	...
	it("should process only added dependencies", async () => {
		mockFetch(
			"https://api.securityscorecards.dev/projects/github.com/test/added-pkg",
			{ score: 5.0 },
		);
	...
	it("should handle concurrent requests efficiently", async () => {
		...
		try {
			...
			assert.strictEqual(
				fetchCallCount,
				10,
				"Should make 10 API calls for 10 different repositories",
			);
			assert(
				endTime - startTime < 500,
				"Should complete in under 500ms due to concurrency (would take 1000ms sequentially)",
			);
		} finally {
			restoreFetch();
		}
