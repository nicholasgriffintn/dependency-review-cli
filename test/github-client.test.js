...
describe('GitHubClient', () => {
  ...
  it('should handle 403 error for dependency graph API', async () => {
    ...
    assert.strictEqual(call.arguments[0], 'GET /repos/{owner}/{repo}/dependency-graph/compare/{basehead}')
  ...
  it('should parse snapshot warnings from response headers', async () => {
    const client = new GitHubClient('fake-token')
    const mockWarnings = 'Some dependencies may not have complete vulnerability data'
    const encodedWarnings = Buffer.from(mockWarnings, 'utf-8').toString('base64')
