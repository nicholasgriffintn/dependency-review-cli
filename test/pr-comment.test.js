...
describe("PrCommenter", () => {
	it("should skip commenting when mode is never", async () => {
		...
		commenter.octokit.rest.issues.listComments = mock.fn(async () => ({
			data: [],
		}));
		...
		assert.strictEqual(
			commenter.octokit.rest.issues.listComments.mock.callCount(),
			0,
	...
	it("should create comment when mode is always", async () => {
		...
		commenter.octokit.rest.issues.createComment = mock.fn(async () => ({
			data: { id: 123 },
		}));
		...
		assert.strictEqual(
			commenter.octokit.rest.issues.listComments.mock.callCount(),
			1,
			"Should check for existing comments",
		);
		assert.strictEqual(
			commenter.octokit.rest.issues.createComment.mock.callCount(),
			1,
			"Should create new comment",
		);

		const createCall =
			commenter.octokit.rest.issues.createComment.mock.calls[0];
	...
	it("should update existing comment", async () => {
		...
		commenter.octokit.rest.issues.updateComment = mock.fn(async () => ({
			data: { id: 456 },
		}));
		commenter.octokit.rest.issues.createComment = mock.fn(async () => ({
			data: { id: 123 },
		...
		assert.strictEqual(
			commenter.octokit.rest.issues.updateComment.mock.callCount(),
			1,
			"Should update existing comment",
		);
		assert.strictEqual(
			commenter.octokit.rest.issues.createComment.mock.callCount(),
			0,
			"Should not create new comment",
		);

		const updateCall =
			commenter.octokit.rest.issues.updateComment.mock.calls[0];
	...
	it("should comment on failure when has issues", async () => {
		...
		assert.strictEqual(
			commenter.octokit.rest.issues.createComment.mock.callCount(),
			1,
			"Should create comment when has issues",
		);
	...
	it("should delete existing comment", async () => {
		...
		commenter.octokit.rest.issues.deleteComment = mock.fn(async () => ({
			data: {},
		}));
		...
		assert.strictEqual(
			commenter.octokit.rest.issues.deleteComment.mock.callCount(),
			1,
			"Should delete comment",
		);

		const deleteCall =
			commenter.octokit.rest.issues.deleteComment.mock.calls[0];
